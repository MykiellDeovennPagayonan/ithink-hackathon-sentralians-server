...
router.post('/chat', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.log(
      'Making OpenAI request with MCP tool:',
    ...
    const chatResponse: ChatResponse = {
      ...
      toolCalls:
        response.output?.filter(
          (item: { type: string }) => item.type === 'mcp_call',
        ) || [],
      approvalRequests:
        response.output?.filter(
          (item: { type: string }) => item.type === 'mcp_approval_request',
        ) || [],
    };
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!approvalRequestId || !previousResponseId) {
        res.status(400).json({
          error: 'Approval request ID and previous response ID are required',
        });
        return;
      }
...
router.get('/tools', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const toolListItems =
      response.output?.filter(
        (item: { type: string }) => item.type === 'mcp_list_tools',
      ) || [];
